...
import ALU._
abstract class AbstractALU(implicit p: Parameters) extends GRVModule {
...
class ALU(implicit p: Parameters) extends AbstractALU()(p) {
    override def desiredName = "MyALU"
    ...
    val cond_out =Mux((io.fn === FN_CZEQZ && in2_not_zero) || (io.fn === FN_CZNEZ && !in2_not_zero), io.in1, 0.U)
    ...
    val out = MuxLookup(io.fn, shift_logic)(Seq(
        FN_ADD -> io.adder_out,
        FN_SUB -> io.adder_out
    ))
